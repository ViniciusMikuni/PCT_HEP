...
def placeholder_inputs(batch_size, num_point, num_features,num_bes):
    pointclouds_lab = tf.placeholder(tf.float32, shape=(batch_size, num_point, num_features))
    pointclouds_higgs = tf.placeholder(tf.float32, shape=(batch_size, num_point, num_features))
    pointclouds_bottom = tf.placeholder(tf.float32, shape=(batch_size, num_point, num_features))
    pointclouds_top = tf.placeholder(tf.float32, shape=(batch_size, num_point, num_features))
    pointclouds_W = tf.placeholder(tf.float32, shape=(batch_size, num_point, num_features))
    pointclouds_Z = tf.placeholder(tf.float32, shape=(batch_size, num_point, num_features))
    bes_vars = tf.placeholder(tf.float32, shape=(batch_size, num_bes))
    mask_padded = tf.placeholder(tf.float32, shape=(batch_size, num_point))
    labels_pl = tf.placeholder(tf.int32, shape=(batch_size))
    return pointclouds_lab,pointclouds_higgs,pointclouds_bottom,pointclouds_top,pointclouds_W,pointclouds_Z, bes_vars,  mask_padded,  labels_pl
...
def get_model_simple(point_cloud, mask,is_training,
                     weight_decay=None, bn_decay=None,scname=''):
    batch_size = point_cloud.get_shape()[0]    
...
def get_extractor(net,is_training, num_class,layer_size,
                  weight_decay=None, bn_decay=None,scname=''):
    
    net = tf_util.fully_connected(net, layer_size, bn=True, is_training=is_training,
                                 activation_fn=tf.nn.relu,
                                 scope=scname+'fc1',bn_decay=bn_decay)
    net = tf_util.dropout(net, keep_prob=0.5, is_training=is_training,
                       scope=scname+'dp1')
